...
def tokenize(args):
    ...
    stories = sorted(os.listdir(stories_dir))
    # make IO list file
    # print("Making list of files to tokenize...")

    import random
    r = str(int(random.random() * 10**10))
    mapping_for_corenlp = "mapping_for_corenlp_" + r + ".txt"

    with open(mapping_for_corenlp, "w") as f:
        for s in stories:
    ...
    command = ['java', 'edu.stanford.nlp.pipeline.StanfordCoreNLP', '-annotators', 'tokenize,ssplit',
               '-ssplit.newlineIsSentenceBreak', 'always', '-filelist', mapping_for_corenlp, '-outputFormat',
               'json', '-outputDirectory', tokenized_stories_dir, " &> /dev/null"]
...
def format_to_bert(args):
    ...
    for corpus_type in datasets:
        a_lst = []
        for json_f in sorted(glob.glob(pjoin(args.raw_path, '*.' + corpus_type + '.*.json'))):
            real_name = json_f.split('/')[-1]
            a_lst.append((corpus_type, json_f, args, pjoin(args.save_path, real_name.replace('json', 'bert.pt'))))
        # print(a_lst, args.raw_path, pjoin(args.raw_path, '.' + corpus_type + '.*.json'))

        pool = Pool(args.n_cpus)
        # for d in pool.imap(_format_to_bert, a_lst):
        for d in pool.map(_format_to_bert, a_lst):
            pass
...
def format_to_lines(args):
    ...
    for corpus_type in datasets:
        ...
        try:
            ...
            corpus_mapping[corpus_type] = {}

    train_files, valid_files, test_files = [], [], []

    # for f in tqdm(glob.glob(pjoin(args.raw_path, '*.json'))):
        # real_name = f.split('/')[-1].split('.')[0]
        # real_name = f.split('/')[-1].replace(".json", "")
    ...
    # for f in tqdm(glob.glob(pjoin(args.raw_path, '*.json'))):
    for i in range(0, len(os.listdir(args.raw_path))):
        f = os.path.join(args.raw_path, str(i) + ".story.json")
        # real_name = f.split('/')[-1].split('.')[0]
        real_name = f.split('/')[-1].replace(".json", "")
        if (real_name in corpus_mapping['valid']):
            valid_files.append(f)
    ...
    for corpus_type in ['train', 'valid', 'test']:
        ...
        for d in pool.map(_format_to_lines, a_lst):
            dataset.append(d)
...
def format_xsum_to_lines(args):
    ...
    for corpus_type in datasets:
        ...
        for d in pool.map(_format_xsum_to_lines, a_lst):
            if (d is None):
                continue
            dataset.append(d)
            if (len(dataset) > args.shard_size):
